...
| 3 | BLIP3o-8B | `BLIP3o-Model-8B` | `mapspatial/backends/veomni/blip3o.py` | `.../ablation_experiment/BLIP3o` | `model.visual()` → `model._llm_forward()` → `lm_head` / DiT→UNet→VAE 2-stage | ✅ | ✅ FIXED | ✅ FIXED | N/A | **Understand**: `Qwen2Attention` 定义 `q_norm`/`k_norm`（checkpoint 无权重→初始化为 0→attention 输出全 0），且 `bias=False`。**修复**: `bias=True`，删除 `q_norm`/`k_norm`。视觉特征 prepend → 用 Qwen2.5-VL 格式 `<|vision_start|><|image_pad|>*N<|vision_end|>`。**Draw**: 使用官方 `EmuVisualGenerationPipeline` + `LuminaFeedForward` monkey-patch + `freqs_cis` 重计算 + diffusers 0.39.0。 |
| 4 | SenseNova-U1-8B-MoT | `SenseNova-U1-8B-MoT` | `mapspatial/backends/veomni/u1.py` | `.../ablation_experiment/SenseNova-U1` | `model.language_model(input_ids, attention_mask, indexes)` / `model.it2i_generate(...)` | ✅ | ✅ FIXED | ✅ FIXED | N/A | **Understand**: M-RoPE 需要 `indexes=[3, seq_len]`。**Draw**: vendor `modeling_neo_chat.py` 有 4 处与 VeOmni 不一致的修改（`_flash_or_sdpa` 添加 GQA 检查→SDPA fallback、`_sdpa_attn_func` 添加 `attn_mask` 参数、`forward_und` 传递 `attn_mask`、`_it2i_prefix_forward` 添加 `gen_indicators`）→ 全部回退为与 VeOmni 完全一致。另外 `image_size=(256,256)` → `(512,512)`，移除 `timestep_shift=3.0`（使用默认值 1）。 |
