"""Verify Bagel chat() works end-to-end after the flash_attn GQA fix."""
import os, sys, time
...
# Test 3: With real image (if available)
print("\n=== Test 3: Real benchmark image ===")
import glob
image_files = glob.glob("/mnt/nas-tbt/caoziqi/code/SpatialIntelligence/MapSpatial-EvalKit/data/sat/t1/direct/*.png") + \
              glob.glob("/mnt/nas-tbt/caoziqi/code/SpatialIntelligence/MapSpatial-EvalKit/data/sat/t1/direct/*.jpg")
if image_files:
    real_img = Image.open(image_files[0])
    print(f"Image: {image_files[0]} size={real_img.size}")
    img = _vae_resize(real_img.convert("RGB"))
    with torch.no_grad():
        response = model.chat(
            tokenizer=tokenizer, new_token_ids=new_token_ids,
            image_transform=image_transform, images=[img],
...
print("\n=== Test 4: Multiple images ===")
img1 = Image.new("RGB", (256, 256), (255, 0, 0))  # Red
img2 = Image.new("RGB", (256, 256), (0, 0, 255))  # Blue
img1 = _vae_resize(img1)
img2 = _vae_resize(img2)
with torch.no_grad():
    response = model.chat(
        tokenizer=tokenizer, new_token_ids=new_token_ids,
