...
def _resolve_rope_theta(config, default=10000.0):
    """Resolve rope_theta from config, handling transformers v5 nesting.

    In transformers v5, rope_theta may be a top-level config attribute
    or nested inside the rope_scaling dict (e.g., Qwen2.5 uses
    ...
    if (
        ...
        return config.rope_scaling["rope_theta"]
    return default
...
class Qwen2RotaryEmbedding(nn.Module):
    def __init__(
        ...
        self.config = config
        # In transformers v5, "default" was removed from ROPE_INIT_FUNCTIONS
        # When rope_type is "default" (no scaling), use None to skip scaling
    ...
    def _dynamic_frequency_update(self, position_ids, device):
        ...
        if seq_len > self.max_seq_len_cached and self.rope_init_fn is not None:  # growth
    ...
    def forward(
        ...
        if use_cache and not isinstance(past_key_values, Cache):
            ...
            if past_key_values is None:
                ...
                # transformers v5 removed from_legacy_cache, use dynamic cache directly
                past_key_values = DynamicCache.from_legacy_cache(past_key_values) if hasattr(DynamicCache, 'from_legacy_cache') else DynamicCache()
                logger.warning_once(
        ...
        if return_legacy_cache:
            # transformers v5 removed to_legacy_cache, return DynamicCache directly
            next_cache = next_cache if isinstance(next_cache, Cache) else DynamicCache.from_legacy_cache(next_cache) if hasattr(DynamicCache, 'from_legacy_cache') else next_cache
