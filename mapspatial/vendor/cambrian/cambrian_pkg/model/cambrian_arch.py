...
class CambrianMetaModel:
    ...
    def initialize_vision_modules(self, model_args, fsdp=None):
        ...
        if pretrain_mm_mlp_adapter is not None:
            logger.info(
                "Loading pretrained mm_projector weights from {}".format(
    ...
    def prepare_inputs_labels_for_multimodal_for_generation(
        ...
        if isinstance(image_features, tuple): # mixed video and images
            ...
            for image_feature, image_size in zip(image_features, image_sizes):
                ...
                if modality == "video":
                    ...
                    if not hasattr(self.get_model().config, "mm_use_im_newline_token") or self.get_model().config.mm_use_im_newline_token: # if not set, default to True
                        # Ensure same device for cat (multi-gpu support)
                        image_feature = image_feature.to(self.model.image_newline.device)
            ...
            if len(image_sizes) == 1: # single images:
                ...
                if not hasattr(self.get_model().config, "mm_use_im_newline_token") or self.get_model().config.mm_use_im_newline_token: # if not set, default to True
                    # Ensure same device for cat (multi-gpu support)
                    image_features = image_features.to(self.model.image_newline.device)
                ...
                # for org_size, per_img_feat in zip(image_sizes, image_features):
                    ...
                    if not hasattr(self.get_model().config, "mm_use_im_newline_token") or self.get_model().config.mm_use_im_newline_token: # if not set, default to True
                        # Ensure same device for cat (multi-gpu support)
                        per_img_feat = per_img_feat.to(self.model.image_newline.device)
            ...
            # Ensure same device for cat (multi-gpu support)
            snapshot_features = snapshot_features.to(self.model.image_newline.device)
            ...
            anyres_features = anyres_features.to(self.model.image_newline.device)
        ...
        # When device_map="auto", vision tower may be on a different GPU; ensure same device for cat
        ...
        if len(image_pos) == 1: # single image or video
            ...
            for _ in range(len(image_pos)):
                ...
                all_input_embeds.append(all_image_features[_].to(target_device))
