...
class MLPAddConnector(nn.Module):
    ...
    def preprocess_spatial_embeds(
        ...
        for i, spatial_embeds_item in enumerate(spatial_embeds_list):
            ...
            # Allow single-frame (image): S=1, npatch_t=1; video needs S = npatch_t * merge_size
            ok = (npatch_t == S // self.visual_temporal_merge_size) or (npatch_t == 1 and S == 1)
            assert ok, "temporal patch number mismatch"
            ...
            # Image (S=1): repeat so merge reshape works (expects S = npatch_t * temporal_merge_size)
            if S == 1:
                spatial_embeds = spatial_embeds.repeat(1, self.visual_temporal_merge_size, 1, 1, 1)
