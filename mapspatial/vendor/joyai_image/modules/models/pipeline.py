...
# In transformers >= 5.0, the @capture_outputs decorator with
# tie_last_hidden_states=True replaces hidden_states[-1] with the
# post-norm last_hidden_state.  In transformers < 5.0, hidden_states[-1]
# was the pre-norm output of the second-to-last layer (manual "before each
# layer" collection).  Use [-2] on >=5.0 to match the <5.0 behaviour.
_USE_PRE_NORM_HIDDEN = version.parse(transformers.__version__) >= version.parse("5.0.0")


def _qwen_last_hidden_state(output):
    """Return the pre-norm second-to-last hidden state from a Qwen3VL forward output.

    This matches the transformers <5.0 behaviour where hidden_states[-1]
    was the pre-norm output of the second-to-last layer.
    """
    hs = output.hidden_states
    if hs is None:
        raise RuntimeError("output_hidden_states=True was not set or hidden_states is None")
    idx = -2 if _USE_PRE_NORM_HIDDEN else -1
    return hs[idx]
...
class Pipeline(DiffusionPipeline):
    ...
    def _get_qwen_prompt_embeds(
        ...
        hidden_states = _qwen_last_hidden_state(encoder_hidden_states)
    ...
    def encode_prompt_multiple_images(
        ...
        last_hidden_states = _qwen_last_hidden_state(encoder_hidden_states)
        if drop_vit_feature:
    ...
    def encode_prompt_images(
        ...
        if images.shape[2] == 1:
            ...
            last_hidden_states = _qwen_last_hidden_state(output_tensor)
