...
class PackedAttentionMoT(nn.Module):
    ...
    def forward_inference(
        ...
        # Expand KV heads for GQA if needed (flash_attn 2.8.4 bug: GQA + Q=1 → NaN)
        if self.num_key_value_groups > 1:
            attn_k = merged_key_states.repeat_interleave(self.num_key_value_groups, dim=1)
            attn_v = merged_value_states.repeat_interleave(self.num_key_value_groups, dim=1)
        else:
            attn_k = merged_key_states
            attn_v = merged_value_states
