...
class Qwen3Attention(nn.Module):
    ...
    def forward_und(self, hidden_states, indexes, attention_mask, past_key_values=None, **kwargs):
        ...
        if flex_mask is not None and _HAS_FLEX_ATTN:
            ...
            # When generating a single new token with past KV cache, use
            # causal=False so the token can attend to all past tokens + itself.
            # Without this, SDPA's causal mask would only let q[0] attend to k[0].
            total_len = key_states.shape[2]
            q_len = query_states.shape[2]
            single_token_gen = total_len > q_len and q_len == 1
            attn_output = _flash_or_sdpa(q, k, v, dropout_p=0.0, softmax_scale=self.scaling, causal=not single_token_gen)
...
class NEOChatModel(PreTrainedModel):
    ...
    def interleave_gen(
        ...
        Generates text tokens autoregressively.  When the model emits
        ``<img>``, switches to flow-matching image generation, then
        re-encodes the generated image back into the KV cache and continues
        text generation.

        Adapted from the official SenseNova-U1 ``interleave_gen``:
        * ``get_conv_template`` replaced by vendored ``_build_t2i_query``
        * raw tensor ``attention_mask`` (not dict ``{"full_attention": ...}``)
        * explicit ``indexes`` for RoPE (no ``current_index`` attribute)
        * ``_build_abs_positions_from_grid_hw`` (vendored name)
        """
        self.img_context_token_id = tokenizer.convert_tokens_to_ids(IMG_CONTEXT_TOKEN)
        self.img_start_token_id = tokenizer.convert_tokens_to_ids(IMG_START_TOKEN)
        self.t_eps = t_eps

        # Handle image_size as a single tuple or a list of tuples
        ...
        # --- Condition prompt ---
        # Build think_content using string concat to avoid HTML interpretation.
        think_content = "<" + "think" + ">\n" if think_mode else "<" + "think" + ">\n\n" + "</" + "think" + ">\n\n"
        query_cond = self._build_t2i_query(
            prompt, system_message=system_message, append_text=think_content
        )

        def replace_image_tokens(query, grid_hw_list):
            for i in range(len(grid_hw_list)):
                num_patch_token = int(
                    grid_hw_list[i][0, 0] * grid_hw_list[i][0, 1] * self.downsample_ratio ** 2
                )
                image_tokens = IMG_START_TOKEN + IMG_CONTEXT_TOKEN * num_patch_token + IMG_END_TOKEN
        ...
        input_embeds_cond, indexes_cond, attn_mask_cond = self._build_it2i_inputs(
        ...
        # --- Text uncondition cache (images only, no text) ---
        question_text_uncond = "<image>" * len(images)
        query_text_uncond = self._build_t2i_query(question_text_uncond)
        query_text_uncond = replace_image_tokens(query_text_uncond, grid_hw)

        input_embeds_tu, indexes_tu, attn_mask_tu = self._build_it2i_inputs(
        ...
        # --- Img (full) uncondition cache (empty prompt + IMG_START_TOKEN) ---
        query_img_uncond = self._build_t2i_query("", append_text=IMG_START_TOKEN)
        input_embeds_iu, indexes_iu, attn_mask_iu = self._build_it2i_inputs(
        ...
        while True:
            ...
            if next_token.item() == self.img_start_token_id:
                ...
                # Build image indexes for all three branches
                ...
                indexes_image_img_uncondition = self._build_t2i_image_indexes(
                    token_h, token_w, indexes_iu[0].max().item() + 1, device=device
                )
                ...
                for step_i in step_iter:
                    ...
                    out_cond = self._t2i_predict_v(
                        image_embeds, indexes_image_condition,
                        past_key_values_cond, t, z,
                    ...
                    if not use_cfg:
                        ...
                        out_img_cond = self._t2i_predict_v(
                            image_embeds, indexes_image_text_uncondition,
                            past_key_values_tu, t, z,
                            image_token_num=token_h * token_w,
                            attn_mask=None,
                            timestep_embeddings=timestep_embeddings,
                            image_size=image_size,
                        ...
                        out_uncond = self._t2i_predict_v(
                            image_embeds, indexes_image_img_uncondition,
                            past_key_values_iu, t, z,
                            image_token_num=token_h * token_w,
                            attn_mask=None,
                            timestep_embeddings=timestep_embeddings,
                            image_size=image_size,
                ...
                def append_image_to_cache(cache, t_idx):
                    ...
                    # Raw tensor mask (vendored code uses raw tensors, not dicts)
                    # Image tokens attend to all past + all image tokens (bidirectional)
                    # but NOT to img_end; img_end attends to everything.
                    mask = torch.zeros(
                        1, 1, tgt_len, past_len + tgt_len, device=device
                    )
