...
class InternVLProcessorKwargs(TypedDict, total=False):
    pass
class InternVLImagesKwargs(TypedDict, total=False):
    pass
...
class InternVLUProcessor(ProcessorMixin):
    ...
    def _merge_kwargs(self, model_processor_class, tokenizer_init_kwargs=None, **kwargs):
        """v5-compatible _merge_kwargs that splits kwargs into text and images buckets without strict TypedDict annotations."""
        text_kwargs = {}
        images_kwargs = {}
        text_keys = {"padding", "return_tensors", "max_length", "truncation", "add_special_tokens"}
        for k, v in kwargs.items():
            if k in text_keys:
                text_kwargs[k] = v
            else:
                images_kwargs[k] = v
        return {"text_kwargs": text_kwargs, "images_kwargs": images_kwargs}
    ...
    def __init__(
        ...
        **kwargs,
        ...
        # In transformers v5, only standard attributes can be passed to super().__init__
        # image_gen_processor is custom, so set it manually after super().__init__()
        # transformers v5: pass positional args for declared attributes
        # Monkey-patch check_argument_for_proper_class to allow InternVLVideoProcessor
        # (custom class that doesn't extend BaseVideoProcessor) for video_processor
        from transformers.processing_utils import ProcessorMixin as _PM
        _orig_check = _PM.check_argument_for_proper_class
        def _patched_check(self_, attribute_name, argument=None):
            if attribute_name == "video_processor":
                self_.__dict__[attribute_name] = argument
                return
            return _orig_check(self_, attribute_name, argument)
        _PM.check_argument_for_proper_class = _patched_check
        try:
            super().__init__(image_processor, tokenizer, video_processor, chat_template=chat_template, **kwargs)
        finally:
            _PM.check_argument_for_proper_class = _orig_check
            ...
            for text_gen, img in zip(ctexts, image):
                if img is not None:
                    ...
                    if drop_cond_type_name in [
                        ...
                        # Ensure pixel_values is a tensor (concatenate_list may return
                        # a flat list of tensors if input is list-of-lists)
                        if not hasattr(image_data["pixel_values"], "shape"):
                            import torch as _torch
                            _pv = image_data["pixel_values"]
                            if len(_pv) > 0 and _torch.is_tensor(_pv[0]):
                                image_data["pixel_values"] = _torch.cat(_pv, dim=0)
                            elif len(_pv) > 0 and isinstance(_pv[0], (list, tuple)):
                                # nested list — try numpy then tensor
                                import numpy as _np
                                image_data["pixel_values"] = _torch.from_numpy(
                                    _np.array(_pv)
                                )
                            else:
                                image_data["pixel_values"] = _torch.tensor(_pv)
                        if generation_mode != "text":
