    ...
    def generate(
        ...
        **generate_kwargs,
        ...
        if pixel_values is not None:
            ...
            # Build input_embeds with image features injected
            ...
            # Patch all attention layers to compute in float32.
            # Root cause: bf16 overflow in attn_weights @ V during decode step
            # with KV cache. The V values (from prefill cache) are large enough
            # that the weighted sum exceeds bf16 range (~65504), producing inf→NaN.
            # Fix: upcast Q/K/V to float32 before attention computation.
            from transformers.models.qwen3.modeling_qwen3 import (
                apply_rotary_pos_emb as _apply_rope,
                eager_attention_forward as _eager_attn,
            )
            import torch.nn as _nn
            orig_forwards = []
            for _layer in self.language_model.model.layers:
                _attn = _layer.self_attn
                _orig = _attn.forward
                def _make_fwd(_a, _o):
                    def _fwd(hidden_states, position_embeddings, attention_mask=None, past_key_values=None, **kwargs):
                        inp_shape = hidden_states.shape[:-1]
                        hid_shape = (*inp_shape, -1, _a.head_dim)
                        q = _a.q_norm(_a.q_proj(hidden_states).view(hid_shape)).transpose(1, 2)
                        k = _a.k_norm(_a.k_proj(hidden_states).view(hid_shape)).transpose(1, 2)
                        v = _a.v_proj(hidden_states).view(hid_shape).transpose(1, 2)
                        cos, sin = position_embeddings
                        q, k = _apply_rope(q, k, cos, sin)
                        if past_key_values is not None:
                            k, v = past_key_values.update(k, v, _a.layer_idx)
                        # float32 attention computation (prevents bf16 overflow)
                        attn_out, _ = _eager_attn(
                            _a, q.float(), k.float(), v.float(),
                            attention_mask.float() if attention_mask is not None else None,
                            dropout=0.0, scaling=_a.scaling,
                        )
                        attn_out = attn_out.to(q.dtype)
                        attn_out = attn_out.reshape(*inp_shape, -1).contiguous()
                        attn_out = _a.o_proj(attn_out)
                        return attn_out, None
                    return _fwd
                _attn.forward = _make_fwd(_attn, _orig)
                orig_forwards.append(_orig)
            ...
            try:
                ...
                for _i, _layer in enumerate(self.language_model.model.layers):
                    _layer.self_attn.forward = orig_forwards[_i]
        return outputs
