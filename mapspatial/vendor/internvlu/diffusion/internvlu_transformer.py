...
class UnifiedMSRoPE(nn.Module):
    ...
    def __init__(self, theta: int, axes_dim: List[int], scale_rope=False):
        ...
        # Store as plain attribute (not a buffer) to avoid transformers v5 meta-device issues.
        # all_inv_freqs is deterministic (computed from theta and axes_dim, not learned),
        # so it doesn't need to be in the state dict or moved with the model.
        self.all_inv_freqs = torch.cat(inv_freqs_list, dim=0).float()

    def get_inv_freq(self, axis_idx, device):
        # Compute directly on the target device to avoid meta/garbage tensor issues
        inv_freqs_list = []
        for dim in self.axes_dim:
            inv_freq = 1.0 / torch.pow(
                self.theta, torch.arange(0, dim, 2, device=device).to(torch.float32).div(dim)
            )
            inv_freqs_list.append(inv_freq)
        all_inv_freqs = torch.cat(inv_freqs_list, dim=0).float()
        start_idx = self.axis_offsets[axis_idx]
        end_idx = self.axis_offsets[axis_idx + 1]
        return all_inv_freqs[start_idx:end_idx]
